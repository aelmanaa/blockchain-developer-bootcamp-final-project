...
contract("Insurance", async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    insurance = await Insurance.new(gateKeeper.address, oracleFacade.address, {
      from: owner,
    });
    ...
    await gateKeeper.addRole(ROLES_CONST.ADMIN_ROLE, defaultAdminRoleId, {
      from: owner,
    });
    ...
    // set roles
    await gateKeeper.addRole(ROLES_CONST.INSURER_ROLE, ROLES_CONST.ADMIN_ROLE, {
      from: admin,
    });
    await gateKeeper.addRole(
      ROLES_CONST.GOVERNMENT_ROLE,
      ROLES_CONST.ADMIN_ROLE,
      { from: admin }
    );
    await gateKeeper.addRole(ROLES_CONST.KEEPER_ROLE, ROLES_CONST.ADMIN_ROLE, {
      from: admin,
    });
    await gateKeeper.addRole(ROLES_CONST.ORACLE_ROLE, ROLES_CONST.ADMIN_ROLE, {
      from: admin,
    });
    await gateKeeper.addRole(ROLES_CONST.FARMER_ROLE, ROLES_CONST.ADMIN_ROLE, {
      from: admin,
    });
    // assign roles
    await gateKeeper.addAssignment(ROLES_CONST.INSURER_ROLE, insurer, {
      from: admin,
    });
    await gateKeeper.addAssignment(ROLES_CONST.GOVERNMENT_ROLE, government, {
      from: admin,
    });
    await gateKeeper.addAssignment(ROLES_CONST.KEEPER_ROLE, keeper, {
      from: admin,
    });
    for (let oracle of oracles) {
      await gateKeeper.addAssignment(ROLES_CONST.ORACLE_ROLE, oracle, {
        from: admin,
      });
    }
    for (let farmer of farmers) {
      await gateKeeper.addAssignment(ROLES_CONST.FARMER_ROLE, farmer, {
        from: admin,
      });
    }
  ...
  describe("Check initial state", () => {
    it("Premium, totalOpenSize, totalOpenContracts , intial balance, minimal reuqired liquidity", async () => {
      let res = await insurance.getNumberClosedContracts(
        season,
        REGIONS_CONST.A
      );
      expect(
        (await insurance.totalOpenSize()).toString(),
        `Initial total open size not correct`
      ).to.equal("0");
      expect(
        (await insurance.totalOpenContracts()).toString(),
        `Initial total open contracts not correct`
      ).to.equal("0");
      expect(
        (await insurance.PERMIUM_PER_HA()).toString(),
        `Premium per HA not correct`
      ).to.equal(web3.utils.toWei("0.15", "ether"));
      expect(
        (await insurance.HALF_PERMIUM_PER_HA()).toString(),
        `Half Premium per HA not correct`
      ).to.equal(web3.utils.toWei("0.075", "ether"));
      expect(
        (await insurance.getBalance()).toString(),
        `Contract balance not correct`
      ).to.equal("0");
      expect(
        (await insurance.minimumAmount()).toString(),
        `Minimum liquidity not correct`
      ).to.equal("0");
    });

    it("Empty data for non existing contract", async () => {
      let res1 = await insurance.getContractData1(
        season,
        REGIONS_CONST.A,
        FARMS_CONST[1]
      );
      let res2 = await insurance.getContractData2(
        season,
        REGIONS_CONST.A,
        FARMS_CONST[1]
      );
      ...
      expect(res1[2].toString(), `Contract state not correct`).to.equal(
        CONTRACT_CONST.DEFAULT
      );
      ...
      expect(res2[5].toString(), `severity not correct`).to.equal(
        SEVERITY_CONST.D
      );
    });
    ...
    it("0 number of closed contracts", async () => {
      ...
      expect(res.toString(), `Number closed contracts not correct`).to.equal(
        "0"
      );
    });
  ...
  describe("Register a contract", () => {
    it("Contract must be active", async () => {
      ...
      await expectRevert(
        insurance.register(season, REGIONS_CONST.A, FARMS_CONST[1], "10", {
          from: farmers[0],
          value: "1",
        }),
    ...
    it("Cover for half of the premium", async () => {
      ...
      await expectRevert(
        ...
        "Not enough money to pay for premium"
      );
    });
    ...
    it("Check for duplicates", async () => {
      ...
      await oracleCore.openSeason(season, { from: keeper });
      const amount = multiplyBigNumbers(halfPremiumPerHa, ha);
      await insurance.register(season, REGIONS_CONST.A, FARMS_CONST[1], "10", {
        from: farmers[0],
        value: amount,
      });
      await expectRevert(
    ...
    it("Registration , data correct", async () => {
      ...
      await oracleCore.openSeason(season, { from: keeper });
      const amount = multiplyBigNumbers(halfPremiumPerHa, ha);
      let contractKey = await insurance.getContractKey(
        season,
        REGIONS_CONST.A,
        FARMS_CONST[1]
      );
      ...
      let trans = await insurance.register(
        season,
        REGIONS_CONST.A,
        FARMS_CONST[1],
        size,
        { from: farmers[0], value: addBigNumbers("10", amount), gasPrice: 0 }
      );

      // check events
      expectEvent(trans, "InsuranceRequested", {
        season: season,
        region: REGIONS_CONST.A,
        farmID: FARMS_CONST[1],
        size: size,
        fee: amount,
        farmer: farmers[0],
        key: contractKey,
      ...
      expect(res2[2].toString(), `totalStaked not correct`).to.equal(
        amount.toString()
      );
      expect(res2[3].toString(), `compensation not correct`).to.equal("0");
      expect(res2[4].toString(), `changeGovernment not correct`).to.equal("0");
      ...
      let numContracts = await insurance.getNumberOpenContracts(
        season,
        REGIONS_CONST.A
      );
      expect(
        numContracts.toString(),
        `Number open contracts not correct`
      ).to.equal("1");

      let elemAt = await insurance.getOpenContractsAt(
        season,
        REGIONS_CONST.A,
        "0"
      );
      expect(elemAt, `Wrong element in open contracts array`).to.equal(
        contractKey
      );

      numContracts = await insurance.getNumberClosedContracts(
        season,
        REGIONS_CONST.A
      );
      expect(
        numContracts.toString(),
        `Number closed contracts not correct`
      ).to.equal("0");
      ...
      expect(
        totalOpenSize.toString(),
        `Initial total open size not correct`
      ).to.equal(size);
      expect(
        totalOpenContracts.toString(),
        `Initial total open contracts not correct`
      ).to.equal("1");

      // check minimum amount = 1open contract*keeper_fee + total size * 2.5(D4)*premium
      let expected = addBigNumbers(
        multiplyBigNumbers(insuranceKeeperFee, totalOpenContracts),
        divideBigNumbers(
          multiplyBigNumbers(size, multiplyBigNumbers(premiumPerHA, "25")),
          "10"
        )
      );
      expect(
      ...
      expect(
        addBigNumbers(balanceAfter, amount).toString(),
        "Balance not correct"
  ...
  describe("Validate a registered contract", () => {
    ...
    beforeEach(async () => {
      ...
      contractKey = await insurance.getContractKey(
        season,
        REGIONS_CONST.A,
        FARMS_CONST[1]
      );
      await insurance.register(season, REGIONS_CONST.A, FARMS_CONST[1], size, {
        from: farmers[0],
        value: addBigNumbers("10", amount),
      });
    });
    ...
    it("Contract must be active", async () => {
      await insurance.switchContractOff({ from: owner });
      await expectRevert(
        insurance.validate(season, REGIONS_CONST.A, FARMS_CONST[1], {
          from: government,
          value: "1",
        }),
        "Contract is currently suspended."
    ...
    it("Season must be open", async () => {
      await expectRevert(
        insurance.validate("2022", REGIONS_CONST.A, FARMS_CONST[1], {
          from: government,
          value: "1",
        }),
        "Season must be open."
    ...
    it("Contract must exist", async () => {
      await expectRevert(
        insurance.validate(season, REGIONS_CONST.A, FARMS_CONST[2], {
          from: government,
          value: "1",
        }),
    ...
    it("Cannot validate twice", async () => {
      await insurance.validate(season, REGIONS_CONST.A, FARMS_CONST[1], {
        from: government,
        value: amount,
      });
      await expectRevert(
    ...
    it("Check data and events", async () => {
      ...
      let trans = await insurance.validate(
        season,
        REGIONS_CONST.A,
        FARMS_CONST[1],
        { from: government, value: amount, gasPrice: 0 }
      );

      // check events
      expectEvent(trans, "InsuranceValidated", {
        season: season,
        region: REGIONS_CONST.A,
        farmID: FARMS_CONST[1],
        totalStaked: multiplyBigNumbers(amount, "2").toString(),
      ...
      expect(res2[1].toString(), `season not correct`).to.equal(season);
      expect(res2[2].toString(), `totalStaked not correct`).to.equal(
        multiplyBigNumbers(amount, "2").toString()
      );
      expect(res2[3].toString(), `compensation not correct`).to.equal("0");
      expect(res2[4].toString(), `changeGovernment not correct`).to.equal("0");
      expect(res2[5].toString(), `severity not correct`).to.equal(
  ...
  describe("Activate a validated contract", () => {
    ...
    it("Contract must be active", async () => {
      await insurance.switchContractOff({ from: owner });
      await expectRevert(
        insurance.activate(season, REGIONS_CONST.A, FARMS_CONST[1], {
          from: insurer,
        }),
        "Contract is currently suspended."
      );
    ...
    it("Season must be open", async () => {
      await expectRevert(
        insurance.activate("2022", REGIONS_CONST.A, FARMS_CONST[1], {
          from: insurer,
        }),
        "Season must be open."
      );
    ...
    it("Contract must exist", async () => {
      await expectRevert(
        insurance.activate(season, REGIONS_CONST.A, FARMS_CONST[2], {
          from: insurer,
        }),
        "Contract do not exist"
      );
    ...
    it("Cannot activate twice", async () => {
      await insurance.activate(season, REGIONS_CONST.A, FARMS_CONST[1], {
        from: insurer,
      });
      await expectRevert(
        insurance.activate(season, REGIONS_CONST.A, FARMS_CONST[1], {
    ...
    it("Check data and events", async () => {
      let trans = await insurance.activate(
        season,
        REGIONS_CONST.A,
        FARMS_CONST[1],
        { from: insurer }
      ...
      // check events
      expectEvent(trans, "InsuranceActivated", {
        season: season,
        region: REGIONS_CONST.A,
        farmID: FARMS_CONST[1],
        insurer: insurer,
  ...
  describe("Process insurance contract contract", () => {
    ...
    it("Contract must be active", async () => {
      await insurance.switchContractOff({ from: owner });
      await expectRevert(
        insurance.process(season, REGIONS_CONST.A, { from: keeper }),
        "Contract is currently suspended."
      );
    });
    ...
    it("Only keeper", async () => {
      await expectRevert(
        insurance.process(season, REGIONS_CONST.A, { from: government }),
    ...
    it("Refund farmer if contract not backed by government", async () => {
      ...
      let trans = await insurance.process(season, REGIONS_CONST.A, {
        from: keeper,
      });
      expectEvent(trans, "InsuranceClosed", {
        season: season,
        region: REGIONS_CONST.A,
        farmID: FARMS_CONST[1],
        size: size,
        farmer: farmers[0],
        insurer: CONST.EMPTY_ADDRESS,
        government: CONST.EMPTY_ADDRESS,
        totalStaked: amount,
        compensation: amount,
        changeGovernment: "0",
        severity: SEVERITY_CONST.D,
        key: contractKey,
      });

      // farmer credited back
      let events = await escrowInsurance.getPastEvents("Deposited", {
        fromBlock: "latest",
      });
      expect(
        isEventFound(events, "Deposited", {
          payee: farmers[0],
          weiAmount: amount.toString(),
        }),
        `Deposited with expected args ${JSON.stringify({
          payee: farmers[0],
          weiAmount: amount.toString(),
        })} not found in ${JSON.stringify(events)}`
      ).to.equal(true);
      ...
      expect(res2[3].toString(), `compensation not correct`).to.equal(
        amount.toString()
      );
      expect(res2[4].toString(), `changeGovernment not correct`).to.equal("0");
      expect(res2[5].toString(), `severity not correct`).to.equal(
      ...
      let elemAt = await insurance.getClosedContractsAt(
        season,
        REGIONS_CONST.A,
        "0"
      );
      expect(elemAt, `Wrong element in closed contracts array`).to.equal(
        contractKey
      );
      ...
      expect(
        totalOpenContracts.toString(),
        `total open contracts not correct`
      ).to.equal("0");
    ...
    it("Refund farmer and government if contract not activated by insurance", async () => {
      ...
      expect(res2[4].toString(), `changeGovernment not correct`).to.equal(
        amount.toString()
      );
      expect(res2[5].toString(), `severity not correct`).to.equal(
        SEVERITY_CONST.D
    ...
    it("Pay farmer correct amount if severity = D4", async () => {
      ...
      await oracleCore.submit(season, REGIONS_CONST.A, SEVERITY_CONST.D3, {
        from: oracles[0],
      });
      await oracleCore.submit(season, REGIONS_CONST.A, SEVERITY_CONST.D4, {
        from: oracles[1],
      });
      await oracleCore.submit(season, REGIONS_CONST.A, SEVERITY_CONST.D2, {
        from: oracles[2],
      });
      await oracleCore.submit(season, REGIONS_CONST.A, SEVERITY_CONST.D2, {
        from: oracles[3],
      ...
      await oracleCore.submit(season, REGIONS_CONST.A, SEVERITY_CONST.D1, {
        from: oracles[6],
      });
      await oracleCore.submit(season, REGIONS_CONST.A, SEVERITY_CONST.D1, {
        from: oracles[7],
      ...
      await oracleCore.submit(season, REGIONS_CONST.A, SEVERITY_CONST.D0, {
        from: oracles[9],
      });
      // close season
      await oracleCore.closeSeason(season, { from: keeper });
      ...
      let totalStaked = addBigNumbers(amount, amount);
      let compensation = divideBigNumbers(
        multiplyBigNumbers(totalStaked, "25"),
        "10"
      );
      let trans = await insurance.process(season, REGIONS_CONST.A, {
        from: keeper,
      });
      expectEvent(trans, "InsuranceCompensated", {
        season: season,
        region: REGIONS_CONST.A,
        farmID: FARMS_CONST[1],
        size: size,
      ...
      expect(
        isEventFound(events, "Deposited", {
          payee: farmers[0],
          weiAmount: compensation.toString(),
        }),
        `Deposited with expected args ${JSON.stringify({
          payee: farmers[0],
          weiAmount: compensation.toString(),
    ...
    it("Pay farmer correct amount if severity = D1", async () => {
      ...
      let totalStaked = addBigNumbers(amount, amount);
      let compensation = divideBigNumbers(
        multiplyBigNumbers(totalStaked, "5"),
        "10"
      );
      let trans = await insurance.process(season, REGIONS_CONST.A, {
        from: keeper,
    ...
    it("No compensation if severity = D0", async () => {
      ...
      expect(
        isEventFound(events, "Deposited", { payee: farmers[0] }),
        `Doesnt expect this event in ${JSON.stringify(events)}`
      ).to.equal(false);
